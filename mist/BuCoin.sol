...
contract Crowdsale is Haltable {
    ...
    function Crowdsale(address _token, uint256 _start, uint256 _end, uint256 _minimumFundingGoal) public {
        require(_token != 0x0);
        require(_start > 0);
        require(_end > 0);
        require(_start < _end);
    ...
    function setPricingStrategyAndTokenLock(PricingStrategy _pricingStrategy, TokenLock _tokenLock) public onlyOwner {
...
contract TokenLock is Ownable {
     ...
     *
     */
    function TokenLock(address _owner, uint256 _freezePeriod, address _token, uint256 _tokensMaxAllocatable) public {
        require(_owner != 0x0);
        require(_token != 0x0);
...
contract BuEarlyRaiserTokenLock is TokenLock {
    ...
    function BuEarlyRaiserTokenLock(address _owner, address _token) public TokenLock(_owner, FREEZE_PERIOD, _token, TOKEN_MAX_HOLD)  {
    }
}
...
contract BuTeamMemberTokenLock is TokenLock {
    ...
    function BuTeamMemberTokenLock(address _owner, address _token) public TokenLock(_owner, FREEZE_PERIOD, _token, TOKEN_MAX_HOLD) {
    }
}
...
contract BuPrivateRaiserPricingStrategy is PricingStrategy {
    ...
    function BuPrivateRaiserPricingStrategy() public {
...
contract BuCoinICO is Crowdsale {
    ...
    function BuCoinICO(address _token, uint256 _end, uint256 _minimumFundingGoal) public Crowdsale(_token, now, _end, _minimumFundingGoal){
    }
